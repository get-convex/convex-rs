...
impl WebSocketWorker {
    ...
    async fn run(
        ...
        loop {
            let e = match worker
                .work(last_close_reason, max_observed_timestamp, client_id.clone())
                .await
            {
                Ok(reconnect) => {
...
impl WebSocketInternal {
    async fn new(
        ...
        let version = client_id.unwrap_or_else(|| format!("rust-{}", VERSION.unwrap_or("unknown")));
        request
            .headers_mut()
            .insert("Convex-Client", version.try_into().context("Bad version")?);
